...
# ==== Cấu hình người dùng ====
CALIB_FILE = "data_calib_new.npz"   # file calib
...
def load_calibration(path: str):
    """Đọc camera_matrix và dist_coeffs từ file .npz"""
...
# Hiệu chỉnh khung hình méo
def undistort_frame(frame, mtx, dist):
    """Undistort frame dùng calib"""
    h, w = frame.shape[:2]
    newK, _ = cv2.getOptimalNewCameraMatrix(mtx, dist, (w, h), 1, (w, h))
    und = cv2.undistort(frame, mtx, dist, None, newK)
    return und
...
class HomographyApp:
    # Khởi tạo
    def __init__(self, calib_file, h_file, cam_index, scale, real_pts_cm):
        ...
        self.mtx, self.dist = load_calibration(self.calib_file)
    ...
    def _mouse_cb(self, event, x, y, flags, param):
        """Callback chuột: chọn điểm hoặc đo tọa độ"""
    ...
    def run(self):
        while True:
            ...
            und = undistort_frame(frame, self.mtx, self.dist)
            disp = cv2.resize(und, None, fx=self.scale, fy=self.scale)
