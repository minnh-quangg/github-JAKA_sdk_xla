...
def detect_marker_pose(timeout_s):
    pipe = rs.pipeline()
    cfg  = rs.config()
...
def pick_objects(P_EE_cam, Rotation_ojb2cam, speed=100, acc=20, tol=1):
    ...
    robot.joint_move_extend(joint_pos=joint_pose, move_mode=0, is_block=True, speed=0.2, acc=0.05, tol=0.1)
    # print(ret)
    # time.sleep(100)
    robot.joint_move_extend(joint_pos=p_home, move_mode=0, is_block=True, speed=0.2, acc=0.05, tol=0.1)
    # print("Robot đã hoàn thành di chuyển!")
    robot.logout()
    return ret
...
if __name__ == "__main__":
    ...
    pose = detect_marker_pose(timeout_s=20.0)
