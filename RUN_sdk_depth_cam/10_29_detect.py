...
def median_depth_mm(depth_mm, cx, cy, k=5):
    h, w = depth_mm.shape[:2]              # if [:3] -> color channel
    r = k // 2
    x1 = max(cx - r, 0); y1 = max(cy - r, 0)
    x2 = min(cx + r + 1, w); y2 = min(cy + r + 1, h)
    roi = depth_mm[y1:y2, x1:x2]
    if roi.size == 0: return 0              # no valid depth value
    valid = roi[roi > 0]                    # point without data
    if valid.size == 0: return 0
    return float(np.median(valid))          # return value: mm (float)
...
class Track:
    ...
    def __init__(self, tid, cx, cy):
        self.tid = tid                      # ID
        self.cx, self.cy = cx, cy           # center (Cx, Cy)
        self.age = 0                        # number of appearances
        self.miss = 0                       # number of times missing
        self.depth_hist = deque(maxlen=20)  # depth history
...
# ==== Manage the track list ==== #
class CentroidTracker:
    def __init__(self, max_distance=60, max_age=30):
        self.max_distance = max_distance    # threshold distance
        self.max_age = max_age              # maxium frames allow to be lost             
        self.tracks = {}
        self.next_id = 1
    @staticmethod
    def _dist(a,b):
    ...
    def update(self, detections):           # Update data, Maintain stable ID; Create and Delete -track-
...
def put(frame, txt, pos, fg=(255,255,255), bg=(0,0,0)):
    #cv2.putText(image, text, org, font, scale, color, thickness, linetype)
    cv2.putText(frame, txt, pos, cv2.FONT_HERSHEY_SIMPLEX, 0.8, bg, 1, cv2.LINE_AA)
    cv2.putText(frame, txt, pos, cv2.FONT_HERSHEY_SIMPLEX, 0.8, fg, 1, cv2.LINE_AA)
...
while True:
    ...
    for det, trk, bb in zip(detections, assigned, bboxes):
        ...
        top_y = y1 - 10
        if top_y > 60:
            ys = [top_y, top_y-14, top_y-28]
        else:
            base = y2 + 20
            ys = [base, base+14, base+28]

        put(frame, f"{label} {conf:.2f}", (base_x, ys[0]))
        put(frame, coord_line, (base_x, ys[2]), (50,200,255))
