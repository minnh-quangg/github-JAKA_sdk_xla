...
if __name__ == "__main__":
    ...
    if pose is None:
        ...
        rx_USC, ry_USC, rz_USC = convert_camera_to_base_rotation (math.degrees(rx_rad), math.degrees(ry_rad), math.degrees(rz_rad))
        ...
        # print(f"Rx={math.degrees(rx_USC):.2f}°, "
        #       f"Ry={math.degrees(ry_USC):.2f}°, "
        #       f"Rz={math.degrees(rz_USC):.2f}°")

        P_cam = np.array([x_cam_mm, y_cam_mm, z_cam_mm])
        R_cam = np.array([rx_USC, ry_USC, rz_USC])
