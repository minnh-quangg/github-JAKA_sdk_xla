...
rs = RealsenseCamera()
depth_intrin = rs.get_intrinsics()
model = YOLO("best.pt")

cv2.namedWindow("Measure Distance (mm)", cv2.WINDOW_NORMAL)
cv2.namedWindow("3D Coordinates", cv2.WINDOW_NORMAL)
...
while True:
    ...
    for det,trk,bb in zip(detections, tracks_assigned, bboxes):
        ...
        Z = d_avg / 1000.0
        delta = f"{271-d_avg}mm" if d_avg>0 else "N/A"

        fx, fy = depth_intrin.fx, depth_intrin.fy
        cx0, cy0 = depth_intrin.ppx, depth_intrin.ppy
        X = (cx - cx0) * Z / fx
        Y = (cy - cy0) * Z / fy

        coord_texts.append(f"{label}: X={X:.1f}mm  Y={Y:.1f}mm  Z={Z:.1f}mm")
    ...
    # === Hiển thị cửa sổ "3D Coordinates" ===
    coord_display = np.zeros((400,600,3), dtype=np.uint8)
    put(coord_display, "Object 3D Coordinates", (20,40), (0,255,255))
    for i, line in enumerate(coord_texts):
        put(coord_display, line, (20, 80 + i*30), (50,200,255))
    cv2.imshow("3D Coordinates", coord_display)
