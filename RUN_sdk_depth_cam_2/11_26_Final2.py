...
def rotation_matrix_to_euler_xyz(R):
    if R[2, 0] < -1.0:      #giới hạn [-1, 1]
        R[2, 0] = -1.0
    if R[2, 0] > 1.0:
        R[2, 0] = 1.0

    ry = math.asin(-R[2, 0])    #tính ry
    cy = math.cos(ry)

    if abs(cy) > 1e-6:  # TH đặc biệt: ry = 0
        rx = math.atan2(R[2, 1], R[2, 2])
        rz = math.atan2(R[1, 0], R[0, 0])
    else:                           
        rx = math.atan2(-R[1, 2], R[1, 1])
        rz = 0.0    # giả sử chỉ quay theo rx 
...
def median_depth_mm(depth_mm, cx, cy, k=5):
    ...
    valid = roi[roi > 0]        # lọc các giá trị < 0 -> tránh gây nhiễu data
    if valid.size == 0:
        return 0.0
    return float(np.median(valid))


# lọc smooth - low pass filter
class DepthSmoother:
    ...
    def update(self, new_val):
        ...
        if self.last_val == 0:
            ...
            # GT_new = GT_ht*(1-alpha) + GT_moi*alpha
...
def detect_marker_pose(timeout_s=10.0):
    ...
    # raw units -> mét
    depth_sensor = profile.get_device().first_depth_sensor() #lấy cảm biến depth
    depth_scale = depth_sensor.get_depth_scale()   # scale mét / raw_unit (depth unit)
    # print("Depth scale (m/unit):", depth_scale)

    # căn chỉnh hệ tọa độ cam (lấy ảnh màu làm gốc)
    align = rs.align(rs.stream.color)

    #lấy thông số nội tại cam
    ...
    try:
        while True:
            ...
            if ids is not None:
                ...
                # hàm estimate pose (tvecs: vector tịnh tiến, rvecs: vector quay)
                rvecs, tvecs, _ = aruco.estimatePoseSingleMarkers(
                    corners, marker_length, camera_matrix, dist_coeffs
                )

                for i, (rvec, tvec, corner) in enumerate(zip(rvecs, tvecs, corners)):
                    # vẽ: red-trục x; green-trục y; blue-trục z
                    cv2.drawFrameAxes(color, camera_matrix, dist_coeffs,
                                      rvec, tvec, 0.03)

                    #tính tâm vật
                    pts = corner[0]
                    cx_pix = int(np.mean(pts[:, 0])) #trung bình cộng tọa độ x -> cx
                    cy_pix = int(np.mean(pts[:, 1])) #trung bình cộng tọa độ y -> cy
                    ...
                    R, _       = cv2.Rodrigues(rvec) # chuyển từ vector xoay -> matrix quay
                    rx, ry, rz = rotation_matrix_to_euler_xyz(R) # chuyển sang góc quay euler
                    # ### Trực
                    rx = (rx - 3.14)
                    if rx < -2* 2.57:
                        rx = rx + 2* 3.14
