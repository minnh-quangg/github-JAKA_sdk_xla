...
def pick_objects(P_EE_cam, Rotation_ojb2cam, speed=100, acc=20, tol=1):
    ...
    p_target=deg_to_rad([61.148, 48.295, 60.023, -1.659, 68.63, -178.1])
    p_temp=deg_to_rad([36.935, 29.451, 61.954, -0.944, 88.952, -187.232])
    # p_truc= [130, 85, 125]
    ...
    P_EE_1= T @ p
    P_EE_2= P_EE_1[:3,0]
    p_truc= [P_EE_2[0],P_EE_2[1],P_EE_2[2]+50]
    ...
    print(P_EE_2,(rx_UCS),(ry_UCS),(rz_UCS))
    print(P_EE_2,math.degrees(rx_UCS),math.degrees(ry_UCS),math.degrees(rz_UCS))
    ...
    # if  rx_UCS < -15 or rx_UCS > 15 or ry_UCS < -15 or ry_UCS > 15:
        
    #     r_USC = [math.radians(-178.693), math.radians(0), rz_UCS]
    #         # move point
    #     P_EE_rxryrz = np.hstack((P_EE_2, r_USC))
    ...
    # else:
    #     # move point
    #     P_truc_rxryrz = np.hstack((p_truc, [rx_UCS, ry_UCS, rz_UCS]))
    #     status1,joint_pose1=robot.kine_inverse(p_home,P_truc_rxryrz)
    ...
    #     robot.joint_move_extend(joint_pos=joint_pose1, move_mode=0, is_block=True, speed=2, acc=0.5, tol=0.1)
    #     time.sleep(1)
    #     robot.joint_move_extend(joint_pos=joint_pose2, move_mode=0, is_block=True, speed=2, acc=0.5, tol=0.1)
    #     # print(ret)
    #     # time.sleep(100)
    # move point
    P_truc_rxryrz = np.hstack((p_truc, [rx_UCS, ry_UCS, rz_UCS]))
    ...
    time.sleep(1)
    robot.joint_move_extend(joint_pos=p_temp, move_mode=0, is_block=True, speed=2, acc=0.5, tol=0.1)
    time.sleep(1)
    robot.joint_move_extend(joint_pos=p_target, move_mode=0, is_block=True, speed=2, acc=0.5, tol=0.1)
    robot.set_digital_output(IO_cabinet, 0, 0)
    time.sleep(1)

    robot.joint_move_extend(joint_pos=p_home, move_mode=0, is_block=True, speed=2, acc=0.5, tol=0.1)
...
if __name__ == "__main__":
    ...
    pose = detect_marker_pose(timeout_s=40.0)
